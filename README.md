...
策略研究的设计、峰谷区域定义、样本外验证和候选晋级标准见：

```text
docs/STRATEGY_DISCOVERY_FRAMEWORK.md
```

可以直接使用 Futu OpenD 的 1 分钟历史数据生成 3 分钟特征、峰谷区域和透明规则候选：

```bash
python -m backlog_screener.cli strategy-research \
  --ticker CRDO \
  --calendar-days 120 \
  --interval-minutes 3
```

也可以使用已有的 1 分钟 OHLCV CSV 做可重复的离线研究：

```bash
python -m backlog_screener.cli strategy-research \
  --ticker CRDO \
  --input-csv outputs/crdo_intraday_features_20260617_232457.csv \
  --interval-minutes 3 \
  --stem crdo_strategy_discovery_v1
```

输出包括完整特征表、峰谷区标签、long/short 两个方向的样本内/样本外规则结果、相邻阈值稳定性和研究摘要。完整方法与首轮 CRDO 结论见：

```text
docs/STRATEGY_DISCOVERY_FRAMEWORK.md
```

规则结果仅用于研究，不代表实盘建议；short 方向在特征阶段使用 signal ticker 的反向收益，晋级前必须再用实际 bear ticker 的 bid/ask 历史验证。

单票研究还会对最稳定的挖掘规则在训练段选择止盈、止损、持仓长度、
冷却时间和每日最大次数，并在最终时间留出段独立验证。跨股票一致性不是
单票晋级要求；每只股票可以保留完全不同的策略机制。

同一套策略族可以批量应用到多只股票，并生成跨股票迁移矩阵：

```bash
python -m backlog_screener.cli strategy-batch-research \
  --tickers CRDO SPCX AAOI \
  --calendar-days 120 \
  --interval-minutes 3 \
  --stem crdo_spcx_aaoi_industry_v1
```

批量报告区分 `portable_candidate`、`portable_experiment`、`ticker_specific`
和 `reject`。所谓可迁移仍只是跨当前样本成立；新股票必须重新通过数据质量、
流动性、时间留出、walk-forward、参数邻域和真实执行价格检查。
